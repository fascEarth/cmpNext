...
async function deleteUser(req, res) {
  try {
    ...
    const rdata = await deleteApi(
      "tag/deletetag/" + data.userid + "?tenantid=" + formedURI,
      token
...
async function getParticularData(req, res) {
  try {
    ...
    const rdata = await getApi(
      "tag/gettag/" + data.userid + "?tenantid=" + formedURI,
      token
    ); // call your api function
    res.status(200).json(rdata);
  } catch (error) {
...
async function getUpdatedataInfo(req, res) {
  try {
    ...
    const rdata = await putApi(
      "tag/updatetag?tenantid=" + formedURI,
      data.data,
      token
    );
    res.status(200).json(rdata);
...
async function getallusercategoryInfo(req, res) {
  try {
    ...
    const formedURI =
      ...
      "&ipaddress=" +
      data.ipaddress;
    const rdata = await getApi("tag/getinitadd?tenantid=" + formedURI, token); // call your api function
...
async function getAlltagsInfo(req, res) {
  try {
    const { data, token } = req.body; // get data from request body
    ...
    const formedURI =
      ...
      "&ipaddress=" +
      data.ipaddress;
    const rdata = await getApi("tag/getalltags?tenantid=" + formedURI, token); // call your api function
