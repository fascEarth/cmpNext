...
function PrivateDetail() {
  ...
  // Assuming slugId is the default networkId
  useEffect(() => {
    setSelectedNetworkId(slugId);
  }, [slugId]);
  ...
  const handleSelectChange = (event) => {
    ...
    router.push(`/surface/networks/privatenet/detail/${selectedId}`);
  };
  ...
  const fetchData = async (tdata) => {
    ...
    const finalData = { data: newData, endPoint: "getAllNetworksFromDetail",  token:tdata.accessToken };
    try {
    ...
    <SurfaceLayout currentPage={2} setBackgrd={true}>
      ...
      <Breadcrumbs aria-label="breadcrumb">
        <Box sx={{ display: "flex" }} className={styles.BreadcrumbsResponsive}>
          <Link href={'/surface/networks/privatenet/list'}>
            <Box
              component="img"
              width={80}
              align="center"
          ...
          <Box className={styles.BreadcrumbSelect}>
            ...
            >
              <Select
              value={selectedNetworkId} // Set the selected value
              ...
              >
               <MenuItem value="" disabled>
                  ...
                  {commonNeList &&
                    commonNeList.map((elem) => (
                      <MenuItem key={elem.networkId} value={elem.networkId}>
                        {elem.networkName}
                      </MenuItem>
                    ))}
