...
function PublicIPDataTable({ expanded, setpubliciptcost, setpubliciptcount }) {
  ...
  const fetchData = async (tdata) => {
    ...
    try {
      ...
      if (data.status === "ok" && data.message) {
        const cdata = data.message["Public IPv4 Address"]
          ? data.message["Public IPv4 Address"]
          : [];
        setData(cdata.details);
        setTotalRecords(parseFloat(cdata.totalCount));

        setpubliciptcount(parseFloat(cdata.totalCount));
        setpubliciptcost(parseFloat(cdata.totalCost.toFixed(2)));
  ...
  useEffect(() => {
    if (!cookies) {
      Cookies.remove("CU_VmPublicIPTableRowsPerPage");
      Cookies.remove("CU_VmPublicIPTablePage");
    }
  }, [cookies]);
  const getPage = () => {
    const CU_VmPublicIPTablePage = Cookies.get("CU_VmPublicIPTablePage");
    if (
      CU_VmPublicIPTablePage &&
      (CU_VmPublicIPTablePage !== undefined || CU_VmPublicIPTablePage !== null)
    ) {
      return CU_VmPublicIPTablePage;
    } else {
      return 0;
    }
  };
  const getTableDatas = () => {
    const CU_VmPublicIPTablePage = Cookies.get("CU_VmPublicIPTablePage");
    const CU_VmPublicIPTableRowsPerPage = Cookies.get(
      "CU_VmPublicIPTableRowsPerPage"
    );
    if (CU_VmPublicIPTablePage && CU_VmPublicIPTablePage !== null) {
      setPage(parseInt(CU_VmPublicIPTablePage, 10));
    }

    if (
      CU_VmPublicIPTableRowsPerPage &&
      CU_VmPublicIPTableRowsPerPage !== null
    ) {
      setRowsPerPage(parseInt(CU_VmPublicIPTableRowsPerPage, 10));
    }
  };
  useEffect(() => {
    getTableDatas();
  ...
  const handleChangePage = (newPage) => {
    Cookies.set("CU_VmPublicIPTablePage", newPage);

    setPage(newPage);
  };

  const handleChangeRowsPerPage = (newRowsPerPage) => {
    Cookies.set("CU_VmPublicIPTableRowsPerPage", newRowsPerPage);
    Cookies.remove("CU_VmPublicIPTablePage");
