...
function VMInstanceDataTable({
  ...
  const fetchData = async (tdata) => {
    ...
    try {
      ...
      if (data.status === "ok" && data.message) {
        const cdata = data.message["VM Instances"]
          ? data.message["VM Instances"]
          : [];
        setData(cdata.details);
        setTotalRecords(parseFloat(cdata.totalCount));

        setvminstancetcount(parseFloat(cdata.totalCount));
        setvminstancetcost(parseFloat(cdata.totalCost).toFixed(2));
  ...
  useEffect(() => {
    if (!cookies) {
      Cookies.remove("CU_VmInstanceTableRowsPerPage");
      Cookies.remove("CU_VmInstanceTablePage");
    }
  }, [cookies]);
  const getPage = () => {
    const CU_VmInstanceTablePage = Cookies.get("CU_VmInstanceTablePage");
    if (
      CU_VmInstanceTablePage &&
      (CU_VmInstanceTablePage !== undefined || CU_VmInstanceTablePage !== null)
    ) {
      return CU_VmInstanceTablePage;
    } else {
      return 0;
    }
  };
  const getTableDatas = () => {
    const CU_VmInstanceTablePage = Cookies.get("CU_VmInstanceTablePage");
    const CU_VmInstanceTableRowsPerPage = Cookies.get(
      "CU_VmInstanceTableRowsPerPage"
    );
    if (CU_VmInstanceTablePage && CU_VmInstanceTablePage !== null) {
      setPage(parseInt(CU_VmInstanceTablePage, 10));
    }

    if (
      CU_VmInstanceTableRowsPerPage &&
      CU_VmInstanceTableRowsPerPage !== null
    ) {
      setRowsPerPage(parseInt(CU_VmInstanceTableRowsPerPage, 10));
    }
  };
  useEffect(() => {
    getTableDatas();
  ...
  const handleChangePage = (newPage) => {
    Cookies.set("CU_VmInstanceTablePage", newPage);
    setPage(newPage);
  };

  const handleChangeRowsPerPage = (newRowsPerPage) => {
    Cookies.set("CU_VmInstanceTableRowsPerPage", newRowsPerPage);
...
export default VMInstanceDataTable;
