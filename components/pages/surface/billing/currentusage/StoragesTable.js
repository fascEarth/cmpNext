...
function StoragesDataTable({ expanded, setstoragestcost, setstoragestcount }) {
  ...
  const fetchData = async (tdata) => {
    ...
    try {
      ...
      if (data.status === "ok" && data.message) {
        const cdata = data.message["Storages"] ? data.message["Storages"] : [];
        setData(cdata.details);
        setTotalRecords(parseFloat(cdata.totalCount));

        setstoragestcount(parseFloat(cdata.totalCount));
        setstoragestcost(parseFloat(cdata.totalCost).toFixed(2));
  ...
  useEffect(() => {
    if (!cookies) {
      Cookies.remove("CU_VmStorageTableRowsPerPage");
      Cookies.remove("CU_VmStorageTablePage");
    }
  }, [cookies]);
  const getPage = () => {
    const CU_VmStorageTablePage = Cookies.get("CU_VmStorageTablePage");
    if (
      CU_VmStorageTablePage &&
      (CU_VmStorageTablePage !== undefined || CU_VmStorageTablePage !== null)
    ) {
      return CU_VmStorageTablePage;
    } else {
      return 0;
    }
  };
  const getTableDatas = () => {
    const CU_VmStorageTablePage = Cookies.get("CU_VmStorageTablePage");
    const CU_VmStorageTableRowsPerPage = Cookies.get(
      "CU_VmStorageTableRowsPerPage"
    );
    if (CU_VmStorageTablePage && CU_VmStorageTablePage !== null) {
      setPage(parseInt(CU_VmStorageTablePage, 10));
    }

    if (CU_VmStorageTableRowsPerPage && CU_VmStorageTableRowsPerPage !== null) {
      setRowsPerPage(parseInt(CU_VmStorageTableRowsPerPage, 10));
    }
  };
  ...
  const handleChangePage = (newPage) => {
    Cookies.set("CU_VmStorageTablePage", newPage);

    setPage(newPage);
  };

  const handleChangeRowsPerPage = (newRowsPerPage) => {
    Cookies.set("CU_VmStorageTableRowsPerPage", newRowsPerPage);
    Cookies.remove("CU_VmStorageTablePage");
...
export default StoragesDataTable;
