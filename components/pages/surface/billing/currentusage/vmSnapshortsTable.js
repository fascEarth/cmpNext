...
function VMSnapshortsDataTable({
  expanded,
  setvmsnapshotcost,
  setvmsnapshotcount,
}) {
  ...
  const fetchData = async (tdata) => {
    ...
    try {
      ...
      if (data.status === "ok" && data.message) {
        const cdata = data.message["VM Snapshots"]
          ? data.message["VM Snapshots"]
          : [];
        setData(cdata.details);
        setTotalRecords(parseFloat(cdata.totalCount));

        setvmsnapshotcount(parseFloat(cdata.totalCount));
        setvmsnapshotcost(parseFloat(cdata.totalCost).toFixed(2));
  ...
  useEffect(() => {
    if (!cookies) {
      Cookies.remove("CU_VmSnapshortsTableRowsPerPage");
      Cookies.remove("CU_VmSnapshortsTablePage");
    }
  }, [cookies]);
  const getPage = () => {
    const CU_VmSnapshortsTablePage = Cookies.get("CU_VmSnapshortsTablePage");
    if (
      CU_VmSnapshortsTablePage &&
      (CU_VmSnapshortsTablePage !== undefined ||
        CU_VmSnapshortsTablePage !== null)
    ) {
      return CU_VmSnapshortsTablePage;
    } else {
      return 0;
    }
  };
  const getTableDatas = () => {
    const CU_VmSnapshortsTablePage = Cookies.get("CU_VmSnapshortsTablePage");
    const CU_VmSnapshortsTableRowsPerPage = Cookies.get(
      "CU_VmSnapshortsTableRowsPerPage"
    );
    if (CU_VmSnapshortsTablePage && CU_VmSnapshortsTablePage !== null) {
      setPage(parseInt(CU_VmSnapshortsTablePage, 10));
    }

    if (
      CU_VmSnapshortsTableRowsPerPage &&
      CU_VmSnapshortsTableRowsPerPage !== null
    ) {
      setRowsPerPage(parseInt(CU_VmSnapshortsTableRowsPerPage, 10));
    }
  };
  useEffect(() => {}, []);

  const handleChangePage = (newPage) => {
    Cookies.set("CU_VmSnapshortsTablePage", newPage);

    setPage(newPage);
  };

  const handleChangeRowsPerPage = (newRowsPerPage) => {
    Cookies.set("CU_VmSnapshortsTableRowsPerPage", newRowsPerPage);
    Cookies.remove("CU_VmSnapshortsTablePage");
...
export default VMSnapshortsDataTable;
