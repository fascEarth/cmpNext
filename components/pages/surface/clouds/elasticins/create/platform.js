...
const Platform = ({ showCinsPlatforms,platformData,setplatformData,setshowCinsdatacenter,setreqParams  }) => {
          ...
          <CardContent sx={{ padding: '24px'}}>
            ...
            {platformData.list.map((item, index) => 
              (
                ...
                onClick={() =>  {
                  if(item.availabilty){
                    setplatformData(prevState => ({...prevState,["defaultId"]:item.id} ))
                    setshowCinsdatacenter(false);
                    setreqParams(prevState => ({ ["platformid"]: item.id }))
                  }
                  
                  
                } }
                key={index} item xs={12} sm={12} md={6} lg={6} xl={6} sx={{cursor:item.availabilty?"pointer":"not-allowed"}} >
                  <Card sx={{position:'relative', borderRadius:'7px', overflow: 'initial',}} className={styles.cardActive}>
                    <CardContent sx={{ padding: '16px !important'}}>
                      <Box sx={{ display: 'flex', alignItems: 'center', }}>
                        <Avatar variant="square" sx={{ width: (index == 1) ? '75px': '100px', height: '75px' }} alt="VMwareCloud" 
                    ...
                    
                        {
                          ...
                          (
                            <Box component="img" width={35} height={35} align="center" className={styles.cardActiveCheck} alt="ActiveCheck" src="/images/pages/surface/clouds/elasticins/create/ActiveCheck.png" />
                          )
                          :
                          ...
                          (
                           ...
                           : (
                            <Typography component="span" align="right" fontSize={14} color="red" className={styles.commingSoon}>
                              Coming Soon
                            </Typography>
                          )
                        }
