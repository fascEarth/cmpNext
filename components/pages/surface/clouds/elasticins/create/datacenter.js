...
const Datacenter = ({
  ...
  setreqParams,
  setdatacenterData,
  setshowCinscloudservertype,
}) => {
  useEffect(() => {
    setcinDatacentersId(datacenterData.defaultId);
  }, [datacenterData.defaultId, setcinDatacentersId]);
        ...
        <Card sx={{ mt: 2, borderRadius: "7px" }}>
          <CardContent sx={{ padding: "24px" }}>
            ...
            <Typography
              ...
              sx={{ pt: 1 }}
            >
              Decide the best region for your instance, check our latency report
              to find suitable datacenter for your current region.
            </Typography>
            <Grid
              ...
              {datacenterData &&
                datacenterData.list.map((item, index) => (
                  <Grid
                    ...
                    onClick={() => {
                      if (item.availabilty) {
                        setcinDatacentersId(item.id);
                        setdatacenterData((prevState) => ({
                          ...prevState,
                          ["defaultId"]: item.id,
                        }));
                        // setshowCinscloudservertype(false);
                    ...
                    xl={6}
                    sx={{
                      cursor: item.availabilty ? "pointer" : "not-allowed",
                    }}
                  >
                    <Card
                      sx={{
                      ...
                      className={
                        datacenterData.defaultId == item.id
                          ? styles.cardActive
                          : ""
                      }
                    >
