...
const PersonalOrgInfoSubmit = ({
  ...
  const getPoifoValid = async (cookieValue) => {
    const newData = {
      account_type: cookieValue.account_type,
      userSerialId: cookieValue.user_serial_id,
      tenantId: cookieValue.tenant_id,
    };
  ...
  const plotPoiorg = (data) => {
    ...
    const industryExists = optionsCrType.some(
      (industry) => industry.value === data.companyType
    );
  ...
  const removeDefaultFile = () => {
    ...
    setpoiOrgstate((prev) => ({ ...prev, file_upload: "" }));
  };
  ...
  const [organizationInfoDisabled, setorganizationInfoDisabled] =
    useState(true);
  ...
  // const [dateofbirthPicker, setdateofbirthPicker] = useState(false);
  const changeDateValueDob = (evt) => {
    const formattedData = evt.format("DD/MM/YYYY");
    ...
    if (evt) {
      errorspoiPersonalInfo.dateOfBirth = false;
    }
  };

  const changeDateValueExpiry = (evt) => {
    const formattedData = evt.format("DD/MM/YYYY");
    setdateOfExpiryErr(false);
    if (formattedData == "Invalid Date") {
      setdateOfExpiryErr(true);
    }
    setpoiPIstate((prevState) => ({
      ...prevState,
    ...
    if (evt) {
      errorspoiPersonalInfo.dateOfExpiry = false;
    }
  };
  ...
  const changeDateValue = (evt) => {
    ...
    setdateOfIssueErr(false);
    if (formattedData == "Invalid Date") {
      setdateOfIssueErr(true);
    }
    setValuepoiPersonalInfo("dateOfIssue", formattedData);
    const unformattedDate = dayjs(formattedData, "DD/MM/YYYY").subtract(
      1,
      "day"
    );
    ...
    if (evt) {
      errorspoiPersonalInfo.dateOfIssue = false;
    }
  };
  ...
  useEffect(() => {
    if (poiPIstate.idProof === "Passport") {
      setidProofMin(5);
    } else {
      setidProofMin(10); // Set the default value if it's not 'Passport'
    }
  }, [poiPIstate.idProof]);
  ...
  const handleInputChange = (evt) => {
    ...
    if (name === "familyName") {
      if (value == "" || value == null) {
        setInputErrorStatus({ ...inputErrorStatus, familyNameStatus: true });
        setInputErrorMsg({
          ...inputErrorMsg,
          familyName: "Family Name is required",
        });
      } else if (value.length <= 1) {
        errorspoiPersonalInfo.familyName = false;
        setInputErrorStatus({ ...inputErrorStatus, familyNameStatus: true });
        setInputErrorMsg({
          ...inputErrorMsg,
          familyName: "",
        ...
        setInputErrorMsg({
          ...inputErrorMsg,
          familyName: "Family name should not exceed 50 characters",
        });
      } else {
        setInputErrorStatus({ ...inputErrorStatus, familyNameStatus: false });
        setInputErrorMsg({ ...inputErrorMsg, familyName: "" });
        errorspoiPersonalInfo.familyName = false;
      }
    }
    ...
    if (name === "firstName") {
      if (value == "" || value == null) {
        setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: false });
        setInputErrorMsg({
          ...inputErrorMsg,
          firstName: "First name is required",
        });
      } else if (value.length < 3) {
        // errorspoiPersonalInfo.firstName = false;
        setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: true });
        setInputErrorMsg({
          ...inputErrorMsg,
          firstName: "First name should atleast 3 characters",
        });
      } else if (value.length > 50) {
        setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: true });
        setInputErrorMsg({
          ...inputErrorMsg,
          firstName: "First name should not exceed 50 characters",
        });
      } else if (value.length > 3) {
        setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: false });
        setInputErrorMsg({
        ...
        setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: false });
        setInputErrorMsg({ ...inputErrorMsg, firstName: "" });
        errorspoiPersonalInfo.firstName = false;
      }
    }
    ...
    if (name === "nationality") {
      if (value) {
        errorspoiPersonalInfo.nationality = false;
      }
    }
    // ------------------------------------IdProof Validation-------------------------------------------------
    ...
    if (name === "idProof") {
      if (value) {
        errorspoiPersonalInfo.idProof = false;
      }
    }

    // ------------------------------------IdProof Validation-------------------------------------------------

    if (name === "purposeOfUse") {
      if (value) {
        errorspoiPersonalInfo.purposeOfUse = false;
      }
    }

    // ------------------------------------Place Of Issuance Validation-------------------------------------------------
    if (name === "placeOfIssuance") {
      if (value) {
        errorspoiPersonalInfo.placeOfIssuance = false;
        setInputErrorStatus({ ...inputErrorStatus, familyNameStatus: false });
        setInputErrorMsg({
          ...inputErrorMsg,
          placeOfIssuance: "",
    ...
    if (name === "idProofNo") {
      if (value == "" || value == null) {
        setInputErrorStatus({ ...inputErrorStatus, passPortStatus: false });
        // errorspoiPersonalInfo.idProofNo = false;
        setIdProofNoError((prevState) => ({
          ...prevState,
          minLength: "ID Number is required",
          pattern: "",
          maxLength: "",
        }));
      } else {
        if (!idProofRegex.test(value)) {
          setIdProofNoError((prevState) => ({
            ...prevState,
            pattern: "Invalid ID Proof",
            minLength: "",
            maxLength: "",
          }));
          // errorspoiPersonalInfo.idProofNo = false;
        } else if (value.length < idProofMin) {
          setInputErrorStatus({ ...inputErrorStatus, passPortStatus: false });
          setIdProofNoError((prevState) => ({
            ...prevState,
            minLength: `ID Proof Number should be at least ${idProofMin} characters`,
            pattern: "",
            maxLength: "",
          }));
          // errorspoiPersonalInfo.idProofNo = false;
        } else if (value.length > idProofMax) {
          setInputErrorStatus({ ...inputErrorStatus, passPortStatus: false });
          setIdProofNoError((prevState) => ({
            ...prevState,
            maxLength: `ID Proof Number should not exceed ${idProofMax} characters`,
            pattern: "",
            minLength: "",
          }));
          // errorspoiPersonalInfo.idProofNo = false;
        } else {
          // If the input is valid, clear all error messages
          setIdProofNoError({
            pattern: "",
            minLength: "",
            maxLength: "",
    ...
    // Clear the input box if "Proof of ID" is changed from "Passport" to something else
    if (name === "idProof" && value !== "Passport") {
      // Assuming you have a state variable for "ID Number"
      setpoiPIstate({ ...poiPIstate, idProofNo: "" });

      // Clear any existing error messages for "ID Number"
      setIdProofNoError({
        pattern: "",
        minLength: "",
        maxLength: "",
  ...
  //     if (value.length < 10) {
  //       setInputError(
  //         `ID Proof Number should be at least ${idProofMin} characters`
  //       );
  //     } else if (value.length > 10) {
  //       setInputError(`ID Proof Number should not exceed 10 characters`);
  //     } else if (value.length === 10) {
  //       setInputError("");
  //       errorspoiPersonalInfo.idProofNo = false;
  ...
  //   dateOfBirth: false,
  // });
  // const handleOpenDatePicker = (name) => {
  //   setopenDatePicker({ ...openDatePicker, [name]: true });
  // };
  // const handleCloseDatePicker = (name) => {
  //   setopenDatePicker({ ...openDatePicker, dateOfBirth: false });
  // };
  ...
  const [dateOfIssueErr, setdateOfIssueErr] = useState(false);
  const [dateOfExpiryErr, setdateOfExpiryErr] = useState(false);
  const [dateOfBirthErr, setdateOfBirthErr] = useState(false);
  const [expiryDateErr, setexpiryDateErr] = useState(false);
  const [issueDateErr, setissueDateErr] = useState(false);
  const onpoiPersonalInfoSubmit = async (data) => {
    ...
    const isPoiPIstateFilled = requiredFields.every((key) => {
      if (key === "dateOfIssue") {
        if (poiPIstate.dateOfIssue == "Invalid Date") {
          return false;
        }
      } else if (key === "dateOfExpiry") {
        if (poiPIstate.dateOfExpiry == "Invalid Date") {
          return false;
        }
      } else if (key === "dateOfBirth") {
        if (poiPIstate.dateOfBirth == "Invalid Date") {
          return false;
        }
    ...
    if (isPoiPIstateFilled) {
      ...
      if (
        inputErrorStatus.familyNameStatus === false ||
        inputErrorStatus.firstNameStatus === false ||
        inputErrorStatus.passPortStatus === false ||
        inputErrorStatus.placeOfIssuanceStatus === false
      ) {
        setpersonalInfoDisabled(true);
      }
      setbillingAddrDisabled(false);
      ...
      setValuePj("billingAddr");
    } else if (!isPoiPIstateFilled) {
      validateDates();
    }
  };
  const validateDates = () => {
    if (poiPIstate.dateOfIssue == "Invalid Date" || !poiPIstate.dateOfIssue) {
      setdateOfIssueErr(true);
    }
    if (poiPIstate.dateOfExpiry == "Invalid Date" || !poiPIstate.dateOfExpiry) {
      setdateOfExpiryErr(true);
    }
    if (poiPIstate.dateOfBirth == "Invalid Date" || !poiPIstate.dateOfBirth) {
      setdateOfBirthErr(true);
    }

    if (poiPIstate.firstName.length > 3) {
      setInputErrorStatus({ ...inputErrorStatus, firstNameStatus: false });
    }
  };
  const validateOrgDates = () => {
    if (poiOrgstate.issueDate == "Invalid Date" || !poiOrgstate.issueDate) {
      setdateOfIssueErr(true);
    }
    if (poiOrgstate.expiryDate == "Invalid Date" || !poiOrgstate.expiryDate) {
      setexpiryDateErr(true);
    }
  };
  const formpoiBillingAddrMethods = useForm();
  const {
    register: registerpoiBillingAddr,
    handleSubmit: handleSubmitpoiBillingAddr,
    formState: { errors: errorspoiBillingAddr },
    reset: resetpoiBillingAddr,
    control: controlpoiBillingAddr,
    setValue: setValuepoiBillingAddr,
  } = formpoiBillingAddrMethods;
  ...
  const [billingErrorsStatus, setBillingErrorsStatus] = useState({
    zipCodeStatus: false,
    postBoxStatus: false,
  });
  ...
  const handleInputChangeBillingAddr = (evt) => {
    ...
    if (name === "buildingNumber") {
      if (value) {
        errorspoiBillingAddr.buildingNumber = false;
      }
    }
    if (name === "street") {
      if (value) {
        errorspoiBillingAddr.street = false;
      }
    }

    if (name == "city") {
      errorspoiBillingAddr.city = false;
      if (value == "Others") {
        setonOtherCity(true);
    ...
    if (name === "country") {
      if (value) {
        errorspoiBillingAddr.country = false;
      }
    }
    // ----------------------------------------ZIP CODE VALIDATION-----------------------------------------------
    if (name === "zipCode") {
      if (value == "" || value == null) {
        setBillingErrorsStatus({ ...inputErrorStatus, zipCodeStatus: false });
        setBillingErrors({
          ...billingErrors,
          zipCode: "ZipCode is Required",
        });
      } else if (value.length < 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, zipCodeStatus: true });
        setBillingErrors({
          ...billingErrors,
          zipCode: "ZipCode should be at least 5 numbers",
        });
      } else if (value.length > 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, zipCodeStatus: true });
        setBillingErrors({
          ...billingErrors,
          zipCode: "ZipCode should not exceed 5 numbers",
        });
      } else if (value.length > 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, zipCodeStatus: true });
        setBillingErrors({
        ...
        errorspoiBillingAddr.zipCode = false;
        setBillingErrorsStatus({ ...inputErrorStatus, zipCodeStatus: false });
        setBillingErrors({ ...billingErrors, zipCode: "" });
      }
    }
    // ----------------------------------------post box CODE VALIDATION-----------------------------------------------
    if (name === "postBox") {
      if (value == "" || value == null) {
        setBillingErrorsStatus({ ...inputErrorStatus, postBoxStatus: false });
        setBillingErrors({
          ...billingErrors,
          postBox: "Postbox is required",
        });
      } else if (value.length < 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, postBoxStatus: true });
        setBillingErrors({
          ...billingErrors,
          postBox: "Post Box should be at least 5 numbers",
        });
      } else if (value.length > 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, postBoxStatus: true });
        setBillingErrors({
          ...billingErrors,
          postBox: "Post Box should not exceed 5 numbers",
        });
      } else if (value.length > 5) {
        setBillingErrorsStatus({ ...inputErrorStatus, postBoxStatus: true });
        setBillingErrors({
        ...
        errorspoiBillingAddr.postBox = false;
        setBillingErrorsStatus({ ...inputErrorStatus, postBoxStatus: false });
        setBillingErrors({ ...billingErrors, postBox: "" });
      }
    }
  };
  ...
  const onpoiBillingAddrSubmit = async (data) => {
    ...
    if (ispoiBIstateFilled) {
      if (cachedInfo.account_type == "legal") {
        // All required key values are filled
        if (
          billingErrorsStatus.zipCodeStatus === false ||
          billingErrorsStatus.postBoxStatus === false
        ) {
          setpersonalInfoDisabled(true);
          setbillingAddrDisabled(true);
  ...
  const {
    register: registerpoiOrgInfo,
    handleSubmit: handleSubmitpoiOrgInfo,
    formState: { errors: errorspoiOrgInfo },
    reset: resetpoiOrgInfo,
    control: controlpoiOrgInfo,
    setValue: setValuepoiOrgInfo,
  } = formpoiOrgInfoMethods;
  ...
  const changeDateValueIssueDate = (evt) => {
    const formattedData = evt.format("DD/MM/YYYY");
    setissueDateErr(false);
    if (formattedData == "Invalid Date") {
      setissueDateErr(true);
    }
    ...
    if (evt) {
      errorspoiOrgInfo.issueDate = false;
    }
  };
  ...
  const changeDateValueExpirydate = (evt) => {
    ...
    if (evt) {
      errorspoiOrgInfo.expiryDate = false;
    }
  };
  console.log(poiPIstate, "START");
  const handleInputChangepoiOrgInfoFileUp = async (newVariable) => {
    ...
    try {
      const { data } = await axios.post(
        ownAPIBaseURL + ownAPIDefaultURL + "files/uploadcrfile",
        formData,
        {
          headers: {
      ...
      if (data.status == "ok") {
        ...
        setpoiOrgstate((prev) => ({ ...prev, ["file_upload"]: fileName }));
  ...
  useEffect(() => {
    if (labelFile) {
      setpoiOrgstate((prev) => ({
        ...prev,
        ["crFilaName"]: poiOrgstate.crFilaName,
      }));
      setpoiOrgstate((prev) => ({
        ...prev,
        file_upload: poiOrgstate.file_upload,
      }));
    }
  }, [labelFile]);
  useEffect(() => {
  ...
  const handleInputChangepoiOrgInfo = (evt) => {
    ...
    if (name === "dataClassification") {
      if (value) {
        errorspoiOrgInfo.dataClassification = false;
      }
    }
    if (name === "purposeOfUse") {
      if (value) {
        errorspoiOrgInfo.purposeOfUse = false;
      }
    }
    if (name === "companyType") {
      if (value) {
        errorspoiOrgInfo.companyType = false;
      }
    }
    if (name === "companyName") {
      if (value) {
        errorspoiOrgInfo.companyName = false;
      }
    }
    if (name === "crNo") {
      if (value) {
        errorspoiOrgInfo.crNo = false;
      }
    }
  ...
  const onpoiOrgInfoSubmit = async (data) => {
    ...
    const ispoiOrgstateFilled = requiredFields.every((key) => {
      if (poiOrgstate.issueDate == "Invalid Date" || !poiOrgstate.issueDate) {
        ...
        poiOrgstate.expiryDate == "Invalid Date" ||
        !poiOrgstate.expiryDate
      ) {
        return false;
      } else if (key === "file_upload" || key === "othersIndustry") {
        return true; // Skip validation for otherCity field
      }
    ...
    if (ispoiOrgstateFilled) {
      if (poiOrgstate.crFilaName == "") {
        toast.warning("Please upload file to continue");
      } else {
        finalSbtPOI();
      }
    } else {
  ...
  const optionsCrType = [
    {
      value: "Consumer & Industrial Products",
      ...
      <Box sx={{ width: "100%" }}>
        <TabContext value={value}>
          <TabList
            ...
            aria-label="Personal / Org Information Tabs"
            TabIndicatorProps={{
              style: {
          ...
          >
            <Tab
              disabled={personalInfoDisabled && true}
              sx={{ color: "#000" }}
              label="PERSONAL INFO"
              value="personalInfo"
              {...a11yProps(0)}
            />
            <Tab
              disabled={billingAddrDisabled && true}
              sx={{ color: "#000" }}
            ...
            {showOrganizationInfo && (
              <Tab
                disabled={organizationInfoDisabled && true}
                sx={{ color: "#000" }}
              ...
              <Grid container direction="row" rowSpacing={1} spacing={5}>
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    label="Family Name"
                    name="familyName"
                    value={poiPIstate.familyName}
                    onChange={handleInputChange}
                    onFocus={handleInputFocus}
                    onKeyPress={handleKeyPress}
                  />
                  {!inputErrorMsg.familyName && (
                    <>
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    label="First Name"
                    name="firstName"
                    value={poiPIstate.firstName}
                    onChange={handleInputChange}
                    onFocus={handleInputFocus}
                    onKeyPress={handleKeyPress}
                    inputProps={{
                      minLength: 3, // Set your minimum length
                    }}
                  />
                  {!inputErrorMsg.firstName && (
                    <>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                  <Grid
                    ...
                    direction="row"
                    rowSpacing={0}
                    spacing={{ xs: 2, sm: 5, md: 2, lg: 2, xl: 2 }}
                  >
                      ...
                      <CssTextField
                        ...
                        onKeyPress={(e) => {
                          if (poiPIstate.idProof === "Passport") {
                            ...
                            poiPIstate.idProof === "Iqama" ||
                            poiPIstate.idProof === "National ID"
                          ) {
                            // Allow only numbers
                            const charCode = e.charCode;
                        ...
                        name="idProofNo"
                      />
                      {!idProofNoError.pattern &&
                        !idProofNoError.minLength &&
                        !idProofNoError.maxLength && (
                          <>
                  ...
                  >
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <Controller
                        ...
                        rules={{
                          required:
                            poiPIstate.dateOfIssue == "Invalid Date"
                        ...
                        render={({ field }) => (
                          <DatePicker
                            ...
                            value={dayjs(poiPIstate.dateOfIssue, "DD/MM/YYYY")}
                  ...
                  >
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <Controller
                        ...
                        rules={{
                          required:
                            poiPIstate.dateOfExpiry == "Invalid Date"
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssFormControl
                    // onClick={() => {
                    //   handleOpenDatePicker("dateOfBirth");
                    // }}
                    sx={{
                      "& .MuiOutlinedInput-notchedOutline": {
                        borderColor: "rgba(0, 0, 0, 0.23) !important",
                  ...
                  >
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <Controller
                        ...
                        rules={{
                          required:
                            poiPIstate.dateOfBirth == "Invalid Date"
                        ...
                        render={({ field }) => (
                          <>
                            <DatePicker
                              // onClose={() => {
                              //   setdateofbirthPicker(false);
                              // }}
                              // open={openDatePicker.dateOfBirth}
                              {...field}
                              // {...registerpoiPersonalInfo("dateOfBirth", {
                              //   required: "Date Of Birth is required",
                              // })}
                              value={dayjs(
                                poiPIstate.dateOfBirth,
                                "DD/MM/YYYY"
                              )}
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssFormControl margin="normal" fullWidth>
                    ...
                    <Select
                      ...
                      placeholder="Select Proof"
                      labelId="demo-simple-select-purposeOfUse-label"
                      id="demo-simple-select-purposeOfUse"
          ...
          <TabPanel value="billingAddr" index={1}>
            <Box
              onSubmit={handleSubmitpoiBillingAddr(onpoiBillingAddrSubmit)}
              id="poibillingaddrsubmit_form"
              component="form"
              autoComplete="off"
              sx={{ mt: 1 }}
            >
              <Grid container direction="row" rowSpacing={1} spacing={5}>
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    value={poiBIstate.buildingNumber}
                    onChange={handleInputChangeBillingAddr}
                    onFocus={handleInputFocusBillingAddr}
                  />
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    value={poiBIstate.zipCode}
                    onChange={(e) => {
                      const sanitizedValue = e.target.value.replace(
                        /[^0-9]{1,5}/g,
                        ""
                      );
                      handleInputChangeBillingAddr(e);
                      setpoiBIstate({ ...poiBIstate, zipCode: sanitizedValue });
                    }}
                    onFocus={handleInputFocusBillingAddr}
                  />
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    onChange={(e) => {
                      ...
                      handleInputChangeBillingAddr(e);
                      setpoiBIstate({ ...poiBIstate, postBox: sanitizedValue });
                    }}
                    onFocus={handleInputFocusBillingAddr}
                  />

                  {!billingErrors.postBox && (
                    <>
          ...
          <TabPanel value="organizationInfo" index={2}>
            <Box
              onSubmit={handleSubmitpoiOrgInfo(onpoiOrgInfoSubmit)}
              id="poiorginfosubmit_form"
              component="form"
              autoComplete="off"
              sx={{ mt: 1 }}
            >
              <Grid container direction="row" rowSpacing={1} spacing={5}>
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssTextField
                    ...
                    value={poiOrgstate.companyName}
                    onChange={handleInputChangepoiOrgInfo}
                    onFocus={handleInputFocuspoiOrgInfo}
                  />
                ...
                <Grid
                  item
                  xs={12}
                  sm={onOtherIndustry ? 3 : 6}
                  md={onOtherIndustry ? 3 : 6}
                  lg={onOtherIndustry ? 3 : 6}
                  xl={onOtherIndustry ? 3 : 6}
                >
                  <CssFormControl margin="normal" fullWidth>
                    <InputLabel id="demo-simple-select-label">
                      Industry
                ...
                {onOtherIndustry && (
                  <Grid item xs={12} sm={3} md={3} lg={3} xl={3}>
                    <CssTextField
                      ...
                      value={poiOrgstate.othersIndustry}
                      onChange={handleInputChangepoiOrgInfo}
                      onFocus={handleInputFocuspoiOrgInfo}
                    />
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  {labelFile ? (
                     ...
                     *.jpeg, *.jpg, *.png, *.pdf"
                              ...
                              />
                              {/** TR 01 */}
                              {poiOrgstate.file_upload && (
                                <Box title={"Remove Document"}>
                                  <Close
                                    onClick={removeDefaultFile}
                                    sx={{
                                      cursor: "pointer",
                  ...
                  >
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <Controller
                        ...
                        control={controlpoiOrgInfo}
                        rules={{
                          required: !poiOrgstate.issueDate
                        ...
                        render={({ field }) => (
                          <>
                            <DatePicker
                              ...
                              // })}
                              value={dayjs(poiOrgstate.issueDate, "DD/MM/YYYY")}
                  ...
                  >
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <Controller
                        ...
                        control={controlpoiOrgInfo}
                        rules={{
                          required: !poiOrgstate.expiryDate
                  ...
                  <Box sx={{ display: "flex", alignItems: "flex-end" }}>
                    <CssFormControl margin="normal" fullWidth>
                      ...
                      <Select
                        ...
                        value={poiOrgstate.dataClassification}
                        onChange={handleInputChangepoiOrgInfo}
                        onFocus={handleInputFocuspoiOrgInfo}
                        MenuProps={MenuProps}
                      >
                ...
                >
                  <Box sx={style}>
                    ...
                    <Typography
                      ...
                      sx={{ mt: 2 }}
                    >
                      Based on the Cloud Computing Regulatory Framework, issued
                      by the Saudi Communication, Information, and Technology
                      Commission; Cloud customers need to choose the appropriate
                      classification of their data as follows:
                    </Typography>
                    <Accordion
                      sx={{ mt: 3 }}
                    ...
                    >
                      ...
                      <AccordionDetails>
                        ...
                        >
                          ...
                          unauthorized access to this data or its disclosure or
                          its content leads to serious and exceptional damage
                          that cannot be remedied or repaired on:
                        </Typography>
                        <Typography
                          component="p"
                          color={"#6b6f82"}
                        ...
                        >
                          ...
                          National interests, including breaching agreements and
                          treaties, harming the Kingdom&apos;s reputation,
                          diplomatic relations and political affiliations, or
                          the operational efficiency of security or military
                          operations, the national economy, national
                          infrastructure, or government business.
                        </Typography>
                        <Typography
                          component="p"
                          color={"#6b6f82"}
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          The performance of government agencies, which is
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          Broader individual health and safety and the privacy
                          of senior officials. Environmental or natural
                          resources
                        </Typography>
                      </AccordionDetails>
                    </Accordion>
                    ...
                    >
                      ...
                      <AccordionDetails>
                        <Typography
                          ...
                          paddingLeft={1}
                        >
                          Data is classified as (Confidential) if unauthorized
                          access to this data or its disclosure or its content
                          leads to serious and exceptional damage that cannot be
                          remedied or repaired on:
                        </Typography>
                        <Typography
                          component="p"
                          color={"#6b6f82"}
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          National interests, including partially harming the
                          Kingdom&apos;s reputation, diplomatic relations and
                          political affiliations, or the operational efficiency
                          of security or military operations, the national
                          economy, national infrastructure, or government
                          business.
                        </Typography>
                        <Typography
                          component="p"
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          Causes a financial loss at the organizational level
                          that leads to bankruptcy, the inability of the
                          entities to perform their duties, a serious loss of
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          Causes serious harm or injury that affects the life of
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          Leads to long-term damage to environmental or natural
                          resources. Investigating major cases as specified by
                          law, such as terrorism financing cases.
                        </Typography>
                      </AccordionDetails>
                    </Accordion>
                    <Accordion
                    ...
                    >
                      ...
                      <AccordionDetails>
                        <Typography
                          ...
                          paddingLeft={1}
                        >
                          Data is classified as (Restricted): If unauthorized
                          access to or disclosure of this data or its content
                          leads to:
                        </Typography>
                        <Typography
                          component="p"
                        ...
                        >
                          ...
                          limited negative impact on the work of government
                          agencies or economic activities in the Kingdom, or on
                          the work of a specific person
                        </Typography>
                        <Typography
                          component="p"
                          color={"#6b6f82"}
                        ...
                        >
                          <FiberManualRecordIcon sx={{ fontSize: "12px" }} />{" "}
                          Limited damage to any entity&apos;s assets and limited
                          loss on its financial and competitive position.
                          Limited damage in the short term to environmental or
                          natural resources.
                        </Typography>
                      </AccordionDetails>
                    </Accordion>
                    ...
                    >
                      ...
                      <AccordionDetails>
                        <Typography
                          ...
                          paddingLeft={1}
                        >
                          Data is classified as (Public) when unauthorized
                          access to or disclosure of this data or its content
                          does not result in any of the effects mentioned above
                          - in the event that there is no effect on the
                          following:
                        </Typography>
                        <Typography
                          component="p"
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <CssFormControl margin="normal" fullWidth>
                    ...
                    <Select
                      ...
                      value={poiOrgstate.purposeOfUse}
                      onChange={handleInputChangepoiOrgInfo}
                      onFocus={handleInputFocuspoiOrgInfo}
                      MenuProps={MenuProps}
                    >
                      ...
                      <MenuItem value="Plants/Factories">
                        Plants/Factories
                      </MenuItem>
